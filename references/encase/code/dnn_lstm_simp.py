...
#                            filters=8, kernel_size=100, strides=2, 
#                            activation=tf.nn.relu,
#                            padding='SAME', use_bias=True, reuse=False)
#    print("conv_1", conv_1.get_shape())
#    pool_1 = tf.layers.max_pooling1d(conv_1, pool_size=10, strides=2)
...
with tf.variable_scope('fc'):  
    ...
    lstm_cell = tf.contrib.rnn.LSTMCell(1, forget_bias=1.0, state_is_tuple=False)
    outputs, states = tf.nn.dynamic_rnn(lstm_cell, features, dtype=tf.float32, time_major = False)
