...
from cfg import ModelCfg, ModelCfg_ns, TrainCfg, TrainCfg_ns  # noqa: F401
...
def _load_record(reader: CR, rec: str, config: CFG) -> Tuple[np.ndarray, np.ndarray]:
    ...
    labels: np.ndarray,
        the labels of the record
    """
    values = reader.load_resampled_data(rec, leads=config.leads, data_format="channel_first", siglen=None)
